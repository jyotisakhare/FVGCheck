...
def check_good_bullish_fvg(data, ticker):
    ...
    for i in range(0, len(data) - 2):
        ...
        distance =  ((thirdCandleLow - firstCandleHigh) /(secondCandleClose - secondCandleOpen)) * 100
...
# Function to fetch data and check bullish FVG for a list of tickers
def analyze_stocks_weekly_fvg(tickers1, checkGoodFVG, trend):
    wfvg_stock_list = []
    no_fvg_stock_list = []
    for ticker in tickers1:
        # print(f"Analyzing {ticker}...")
        # Fetch stock data from Yahoo Finance
        data = yf.download(ticker, start, end, interval="1wk")
...
# brearish daily
analyze_stocks_daily_fvg(get_tickers("MINE"), "1d", checkGoodFVG=True,
